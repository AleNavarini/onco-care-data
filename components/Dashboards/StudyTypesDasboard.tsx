...
export default function StudyTypesDasboard(props: Props) {
    ...
    const addStudyType = (studyType: StudyType) => setStudyTypes((prevStudyTypes) => [...prevStudyTypes, studyType])

    const updateStudyType = (studyType: StudyType) => {
        setStudyTypes((prevStudyTypes) => prevStudyTypes.map((st: StudyType) => {
    ...
    const deleteStudyType = async (studyType: StudyType) => {
        ...
        if (response.status === 200) {
            setStudyTypes((prevStudyTypes) => prevStudyTypes.filter((d: StudyType) => d.id !== studyType.id))
        }
    }
    ...
    return (
        ...
        >
            ...
            >
                <thead>
                    <tr>
                        <th style={{ width: 100, textAlign: 'center', paddingLeft: 20, verticalAlign: 'middle' }}>Tipo de Estudio</th>
                ...
                <tbody>
                    {studyTypes && studyTypes.length > 0 && studyTypes.map((studyType: StudyType) => (
                        <tr key={studyType.id.toString()}>
                            ...
                            <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                ...
                                <IconButton color="neutral" variant="plain" onClick={() => deleteStudyType(studyType)}>
                                    <DeleteIcon />
                                </IconButton>
                            </td>
