...
export default function TreatmentTypesDasboard(props: Props) {
    const [treatmentTypes, setTreatmentTypes] = useState<TreatmentType[]>(props.treatmentTypes)
    const [editTreatmentType, setEditTreatmentType] = useState<TreatmentType | null>(null)
    const [newModalOpen, setNewModalOpen] = useState<boolean>(false);
    const [editModalOpen, setEditModalOpen] = useState<boolean>(false);

    const addTreatmentType = (treatmentType: TreatmentType) => setTreatmentTypes((prevTreatmentTypes) => [...prevTreatmentTypes, treatmentType])

    const updateTreatmentType = (treatmentType: TreatmentType) => {
        setTreatmentTypes((prevTreatmentTypes) => prevTreatmentTypes.map((tt: TreatmentType) => {
    ...
    const deleteTreatmentType = async (treatmentType: TreatmentType) => {
        ...
        if (response.status === 200) {
            setTreatmentTypes((prevTreatmentTypes) => prevTreatmentTypes.filter((tt: TreatmentType) => tt.id !== treatmentType.id))
        }
    }
    ...
    return (
        ...
        >
            ...
            >
                <thead>
                    <tr>
                        <th style={{ width: 100, textAlign: 'center', paddingLeft: 20, verticalAlign: 'middle' }}>Tipo de Tratamiento</th>
                ...
                <tbody>
                    {treatmentTypes && treatmentTypes.length > 0 && treatmentTypes.map((treatmentType: TreatmentType) => (
                        <tr key={treatmentType.id.toString()}>
                            ...
                            <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                ...
                                <IconButton color="neutral" variant="plain" onClick={() => deleteTreatmentType(treatmentType)}>
                                    <DeleteIcon />
                                </IconButton>
                            </td>
