...
export default function TreatmentForm({ buttonText, patientId, setModalOpen, oldTreatment, handler }: Props) {
    ...
    const treatmentTypeAttributes = treatmentTypes?.filter((tt: FullTreatmentType) => tt.id.toString() === selectedTreatmentType)[0]?.attributes
    const treatmentTypeResults = treatmentTypes?.filter((tt: FullTreatmentType) => tt.id.toString() === selectedTreatmentType)[0]?.results
        ...
        >
            <form onSubmit={handleSubmit(onSubmit)}>
                <Stack spacing={2}>
                    ...
                    >
                        {treatmentTypes && treatmentTypes.map((treatmentType: TreatmentType) => (
                            <Option
