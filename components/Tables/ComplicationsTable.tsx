...
export default function ComplicationsTable({ treatmentId, complications: initialComplications }: Props) {
    const [complications, setComplications] = useState<Complication[]>(initialComplications ? initialComplications : [])
    const [editComplication, setEditComplication] = useState<Complication | null>(null)
    const [newModalOpen, setNewModalOpen] = useState<boolean>(false);
    const [editModalOpen, setEditModalOpen] = useState<boolean>(false);

    const addComplication = (complication: Complication) => setComplications((prevComplications) => [...prevComplications, complication])

    const updateComplication = (complication: Complication) => {
        setComplications((prevComplications) => prevComplications.map((s: Complication) => {
    ...
    const deleteComplication = async (complication: Complication) => {
        ...
        if (response.status === 200) {
            setComplications((prevComplications) => prevComplications.filter((s: Complication) => s.id !== complication.id))
        }
    }
    return (
        <Sheet
        ...
        >
            ...
            >
                    ...
                    <tr>
                        {
                            ...
                            <>
                                <th style={{ textAlign: 'center', verticalAlign: 'middle' }}>Tiempo</th>
                                <th style={{ textAlign: 'center', verticalAlign: 'middle' }}>Tipo</th>
                                <th style={{ textAlign: 'center', verticalAlign: 'middle' }}>Transfusiones</th>
                ...
                <tbody>
                    {complications && complications.length > 0 && complications.map((complication: Complication) => {
                        return (
                            <tr key={complication?.id.toString()}>
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    <Typography fontWeight="md">{complication?.time}</Typography>
                                </td>
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    <Typography noWrap fontWeight="md">{complication?.type}</Typography>
                                </td>
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    <Typography noWrap fontWeight="md">{complication?.transfusions}</Typography>
                                </td>
