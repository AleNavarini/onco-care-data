...
export default function PreviousSurgeriesTable({ patientId, previousSurgeries: initialPreviousSurgeries }: Props) {
    const [previousSurgeries, setPreviousSurgeries] = useState<PreviousSurgery[]>(initialPreviousSurgeries)
    const [editPreviousSurgery, setEditPreviousSurgery] = useState<PreviousSurgery | null>(null)
    const [newModalOpen, setNewModalOpen] = useState<boolean>(false);
    const [editModalOpen, setEditModalOpen] = useState<boolean>(false);

    const addPreviousSurgery = (previousSurgery: PreviousSurgery) => setPreviousSurgeries((prevSurgeries) => [...prevSurgeries, previousSurgery])

    const updatePreviousSurgery = (previousSurgery: PreviousSurgery) => {
        setPreviousSurgeries((prevSurgeries) => prevSurgeries.map((ps: PreviousSurgery) => {
    ...
    const deletePreviousSurgery = async (previousSurgery: PreviousSurgery) => {
        const response = await fetch(`/api/previous-surgeries/${previousSurgery.id}`, {
            method: "DELETE",
            headers: {
        ...
        if (response.status === 200) {
            setPreviousSurgeries((prevSurgeries) => prevSurgeries.filter((ps: PreviousSurgery) => ps.id !== previousSurgery.id))
        }
    }
    return (
        <Sheet
        ...
        >
            ...
            >
                ...
                <tbody>
                    {previousSurgeries && previousSurgeries.length > 0 && previousSurgeries.map((previousSurgery: PreviousSurgery) => (
                        <tr key={previousSurgery?.id.toString()}>
                            <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                <Typography fontWeight="md">{previousSurgery?.surgeryType}</Typography>
                            </td>
                            <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                <Typography noWrap fontWeight="md">{previousSurgery?.observations}</Typography>
                            </td>
                            ...
                            <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                ...
                                <IconButton
                                    ...
                                    variant="plain"
                                    onClick={() => {
                                        var result = confirm("Seguro que queres borrar la cirugia?");
                                        if (!result) return
