...
export default function StagingTable({ patientId, stagings: initialStagings }: Props) {
    ...
    const addStaging = (staging: Staging) => setStagings((prevStagings) => [...prevStagings, staging])

    const updateStaging = (staging: Staging) => {
        setStagings((prevStagings) => prevStagings.map((s: Staging) => {
    ...
    const deleteStaging = async (staging: Staging) => {
        ...
        if (response.status === 200) {
            setStagings((prevStagings) => prevStagings.filter((s: Staging) => s.id !== staging.id))
        }
    }
    return (
        <Sheet
                    ...
                    <tr>
                        {
                            ...
                            <>
                                ...
                                <th style={{ textAlign: 'center', verticalAlign: 'middle' }}>Figo</th>
                    ...
                    {stagings && stagings.length > 0 && stagings.map((staging: Staging) => {
                        ...
                        return (
                            <tr key={staging.id.toString()}>
                                ...
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    <Typography noWrap fontWeight="md">{staging.type}</Typography>
                                </td>
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    <Typography noWrap fontWeight="md">{staging.figo}</Typography>
                                </td>
                                ...
                                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                                    ...
                                    <IconButton
                                        ...
                                        variant="plain"
                                        onClick={() => {
                                            var result = confirm("Quiere borrar la estadificacion?");
                                            if (!result) return
