...
export default function StudyTypeAttributesDashboard(props: Props) {
  const [studyTypeAttributes, setStudyTypeAttributes] = useState<
    StudyTypeAttribute[]
  >(props.studyTypeAttributes);
  const [editStudyTypeAttribute, setEditStudyTypeAttribute] =
    useState<StudyTypeAttribute | null>(null);
  const [newModalOpen, setNewModalOpen] = useState<boolean>(false);
  const [editModalOpen, setEditModalOpen] = useState<boolean>(false);

  const addStudyTypeAttribute = (studyTypeAttribute: StudyTypeAttribute) =>
    setStudyTypeAttributes((prevStudyTypeAttributes) => [
      ...prevStudyTypeAttributes,
      studyTypeAttribute,
    ]);

  const updateStudyTypeAttribute = (studyTypeAttribute: StudyTypeAttribute) => {
    setStudyTypeAttributes((prevStudyTypeAttributes) =>
      prevStudyTypeAttributes.map((rf: StudyTypeAttribute) => {
  ...
  const deleteStudyTypeAttribute = async (
    ...
    if (response.status === 200) {
      setStudyTypeAttributes((prevStudyTypeAttributes) =>
        prevStudyTypeAttributes.filter(
          (rf: StudyTypeAttribute) => rf.id !== studyTypeAttribute.id,
        ),
      );
    }
  };
